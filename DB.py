import MySQLdb, SocketUtils as SU
...
def signup(csocket, thread):
    ack = [-1]
    while ack[0]==-1:
        username = SU.receiveString(csocket, thread)
        if thread.dc:
            return [-1]
...
def loginToGame(csocket, thread):
    ...
    try:
        numplayers, username, roomname = SU.receiveArr(csocket, thread)
        print("Iam here")
    except:
        thread.dc = True
    if thread.dc:
